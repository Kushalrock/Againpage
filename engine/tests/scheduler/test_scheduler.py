...
def test_next_due_subsequent_uses_cadence_gap():
    sched = Scheduler(repo=None, queue=None)
    nd = sched.next_due(_s("daily", time(7)), now=_now(2026,6,30,9), last_issue_date=date(2026,6,30))
    assert nd == datetime(2026,7,1,7,0)                     # daily → tomorrow
    nd = sched.next_due(_s("weekly", time(8)), now=_now(2026,6,30,9), last_issue_date=date(2026,6,30))
