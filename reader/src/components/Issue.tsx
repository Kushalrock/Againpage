    ...
    <article style={{ background: color.paper, padding: '56px clamp(28px,6vw,88px) 64px' }}>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <EditionHead dek={content.dek} title={content.title}
          standfirst={content.standfirst} sources={content.sources} onOpenNote={setOpenNote} />
        <Lead source={content.lead} />
        <Connections items={content.connections.slice(0, c.connections)} />
        {c.summaries && <StandaloneSummaries items={content.standalone_summaries} onOpenNote={setOpenNote} />}
      ...
      <NoteDrawer title={openNote} onClose={() => setOpenNote(null)} />
    </article>
  )
}
